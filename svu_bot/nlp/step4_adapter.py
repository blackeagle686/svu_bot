...
class OutputAdapter:
    ...
    @staticmethod
    def process(llm_output: str, prompt: str = None) -> str:
        # 1. Remove any leaked internal data first
        clean = OutputAdapter.sanitize(llm_output)

        # If sanitization removed everything, provide a helpful, non-blaming fallback.
        if clean is None:
            fallback = (
                "I couldn't find a direct match in the university documents. "
                "I can still help — please either rephrase your question or confirm that you "
                "want an answer based on general knowledge, and I'll provide a full response."
            )
            return fallback
